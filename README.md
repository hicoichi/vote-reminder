...
EPIC-01〜10のユーザーストーリーを実装済みです。実際の通知配信（メール/プッシュ）や外部の選挙データ連携（選挙・投票所・候補者情報は管理画面から手動登録する想定）は行っておらず、ブラウザから一通り操作できるPoCにとどまります。
...
画面上部で地域（郵便番号）を登録したあと、下部の「管理者用: 選挙・投票所などのデータ登録」から選挙・投票所・期日前投票所・候補者などのデータを登録すると、上のタブ（自分の選挙／投票所／期日前投票／通知／投票記録／候補者・公報・結果／履歴）から一通りの機能を確認できます。ポート番号は `python3 -m app.server 8080` のように引数で変更できます。
...
全コマンドは `python3 -m app.cli --help` で確認できます。DBファイルは既定で `data/vote_reminder.db`（`VOTE_REMINDER_DB` 環境変数で変更可、CLIとサーバーで共有）に作成されます。
